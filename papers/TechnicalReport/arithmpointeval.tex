...
of both the  pointer variables an the correctness of all the substractions between pointer expressions.
In order to be able to compute the vality of an pointer arithmetic expression,
one need to associate to each integer variable $i$, resp. to each pointer variable $x$, a variable $\ival{i}$, resp. $\ival{x}$, all ranging in $\lbrace \rbrace \true, \false, \dk$.
