...
of both the  pointer variables  as well as the correctness of all the substractions between pointer expressions. The validity of a pointer expression, in a context $\phi$, is recusively evaluated as : 
$$
\begin{array}{|lll|}
\hline
 \isvalid{\phi}{n} & := & \true \\
\isvalid{\phi}{i} & := & \ival{i} \\
...
\end{array} \right . \\
&&\\
\isvalid{\phi}{I_1\oplus I_2} & := & \isvalid{\phi}{I_1}\wedge \isvalid{\phi}{I_2}, \oplus \in \lbrace + , \times \rbrace \\
&&\\
...
Here, $\ival{i}$ and $\ival{x}$ are counter associated to abstract the fact that
