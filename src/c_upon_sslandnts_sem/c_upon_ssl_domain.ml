...
let ndetval_list_of_il_arg_list l =
  (* Constructors ILScalArg and IlPtrArg are defined in
     module 
     Compilation_utils_types
...
let affect_int_val_upon_sslv ((lv , off) : Cil_types.lval) (expr : Cil_types.exp) 
	  ...
	  match ( Composite_types.is_integer_type type_of_lval) with 
		...
		begin
		  ...
		  (*let cnt_affect = CntAffect(nts_lvar,cnt_expr)*)
			  ...
			  let cnt_affect = Flatac_ndet_nts_support.make_nd_guard_of_det_relation
			    cnt_affect in
  ...
  (***********Transition for valid memory access in expr parameter *******************)
	...
	let global_succs_guard = 
	  Flatac_ndet_nts_support.make_nd_guard_of_det_relation
	  success_cnd
	in
	...
	let failure_cnd = Nts_generic.neg_of_genrel success_cnd in
	
	let failure_guard = 
	  Flatac_ndet_nts_support.make_nd_guard_of_det_relation
	    failure_cnd in 
	(*Nts_generic.neg_cond_in_guard global_succs_guard in*)
...
let affect_ptr_upon_sslv ( (lv,off) : Cil_types.lval)  (expr : Cil_types.exp) mid (sslv : ssl_validity_absdom ) =
  ...
  try
	...
	let cnd_of_rhs_access = Guard_of_mem_acces.cnt_guard_of_mem_access
	  sslv expr in
	let guard_of_rhs_mem_access =
	  Flatac_ndet_nts_support.make_nd_guard_of_det_relation 
	    cnd_of_rhs_access in
	let cnd_of_broken_mem_access = Nts_generic.neg_of_genrel  
	  cnd_of_rhs_access in
	let guard_of_broken_mem_access = 
	  Flatac_ndet_nts_support.make_nd_guard_of_det_relation
	    cnd_of_broken_mem_access
	in
	let broken_mem = 
	  Ssl_valid_abs_dom.create_errorstate_validity_abstdomain () in
	...
	transit_list
      ...
      let pvar_left = 
	Ast_goodies.get_pvar_from_exp_node  (Lval(lv,off)) in
	...
	sslv expr in
      ...
      let guard_of_good_mem_access = Flatac_ndet_nts_support.make_nd_guard_of_det_relation condition in 
(*CntGuard(CntBAnd(cnd_of_rhs_access,cnd_of_lhs_access))*) 
	...
	sslv pvar_left affect_validity_of_pvar pvar_loc_info
      ...
      let guard_of_mem_broken = Flatac_ndet_nts_support.make_nd_guard_of_det_relation cnd_of_mem_broken in
	...
	CntGuard( cnd_of_mem_broken)
      in*)
      let affect_off = Flatac_ndet_nts_support.make_nd_guard_of_det_relation affect_off in
      let copy_valaff_tolhs = Flatac_ndet_nts_support.make_nd_guard_of_det_relation copy_valaff_tolhs in
...
let free_upon_sslv (pvar : ptvar)(sslv : ssl_validity_absdom ) =
  ...
  try
	...
	in
	let eq_zero_transition =   Flatac_ndet_nts_support.make_nd_guard_of_det_relation
	  eq_zero_guard 
	in
	let non_zero_guard = Nts_generic.neg_of_genrel eq_zero_guard
	in
	let neq_zero_transition = Flatac_ndet_nts_support.make_nd_guard_of_det_relation
	  non_zero_guard
	in
...
let r_malloc_succ ( lhs : Cil_types.lval option ) sslv (mid: global_mem_manager ) (scal_param : c_scal) =
  ...
  match lhs with
	...
	let valid_var_aff_guard = 
	  Flatac_ndet_nts_support.make_nd_cnt_bool_of_nts_genrel
	    valid_var_aff_guard in
	(*CntAffect( valid_var_cnt_aff , CntCst(My_bigint.one)) 
	  
	  in*)

	let interpret_gt_zero =
	  Flatac_ndet_nts_support.guard_nd_gt_zero interpret_param
	(*Nts_generic.guard_gt_aexpr interpret_param
	  Nts_generic.zero *)
	  
	in
	let sizeof_param = Flatac_ndet_nts_support.arithm_value_of_ndsupport_or_fails interpret_param in
	let list_locvar_cnt_affect =  
	  Guard_of_mem_acces.make_size_locvar_genrel l mid sizeof_param 
	in 
	let list_locvar_cnt_affect = 
	  Flatac_ndet_nts_support.make_nd_cnt_bool_of_nts_genrel 
	    list_locvar_cnt_affect 
	in
	let cnt_ptvar_offset =  make_offset_locpvar lhs_pvar 
	in
	(*
	...
	*)
	...
	let zero_pvar_offset = 
	  Flatac_ndet_nts_support.make_nd_cnt_bool_of_nts_genrel zero_pvar_offset
	in
	let transition_op = 
	  Flatac_ndet_nts_support.and_of_nd_genrel zero_pvar_offset 
	  interpret_gt_zero 
	in
	let transition_op = 
	  Flatac_ndet_nts_support.and_of_nd_genrel list_locvar_cnt_affect transition_op 
	in
	let transition_op =
	  Flatac_ndet_nts_support.and_of_nd_genrel valid_var_aff_guard transition_op in
	
	let transition_op = 
	  Flatac_ndet_nts_support.make_nd_guard_of_relation transition_op in
	...
	in
	let locvar_cnt_affect = 
	  make_size_locvar_genrel l mid interpret_param_size 
	in
	let locvar_cnt_affect = 
	  Flatac_ndet_nts_support.make_nd_cnt_bool_of_nts_genrel 
	    locvar_cnt_affect 
	in
	let global_guard = 
	  Flatac_ndet_nts_support.and_of_nd_genrel locvar_cnt_affect
	    interpret_gt_zero 
	in
	let global_trans_guard = 
...
let r_malloc_neg_or_zero_arg ( lhs : Cil_types.lval option ) sslv  (mid: global_mem_manager ) (scal_param : c_scal) =
  ...
  match lhs with
	...
	begin
	  ...
	  let pvar = get_pvar_from_exp_node (Lval(lv,off)) in
	  let interpret_param = 
	    interpret_c_scal_to_cnt new_abstract.ssl_part 
	    scal_param 
	  in 
	  let aff_to_nil = Pointsnil(pvar) in
	  and_atomic_ptnil aff_to_nil new_abstract.ssl_part;
	  let guard_leq_zero = 
	    Flatac_ndet_nts_support.guard_nd_leq_zero interpret_param 
	  in
	  (*
	  ...
	  *)
	  ...
	  in
	  let general_cond = 
	    Flatac_ndet_nts_support.and_of_nd_genrel guard_leq_zero 
	      invalid_aff_lhs 
	  in
	  let general_guard = 
	    Flatac_ndet_nts_support.make_nd_guard_of_relation 
	  ...
	  let ret_list = (new_abstract, general_guard :: [])::[]  in
	  ret_list
	end
    | None ->
      	begin
	  let new_abstract = copy_validity_absdomain sslv in
	  let interpret_param =  
	    try_interpret_cscal_from_ndet_supp 

	      new_abstract.ssl_part 
	      scal_param in 
	  let guard_leq_zero = 
...
let r_malloc_neg_or_zero_arg_withvalidityguard (lhs : Cil_types.lval option ) sslv  (mid: global_mem_manager ) (scal_param : c_scal) =
  ...
  match lhs with 
	...
	begin
	  ...
	  let guard = Flatac_ndet_nts_support.make_nd_guard_of_det_relation guard in
	...
	in 
	let guard = 
	  Flatac_ndet_nts_support.and_of_nd_genrel 
	    validity_guard_cnt interpret_leq_zero
	in
	let guard = Flatac_ndet_nts_support.make_nd_guard_of_relation guard in
      (*CntGuard(CntBAnd(validity_guard_cnt,interpret_leq_zero)) *)
	let ret_list = ((new_abstract, (guard :: []))::[] ) 
	in
	ret_list	
		    ...
		    begin
			  ...
			  TPtr(_,_) -> (*|  TNamed(_,_) ->*)
				...
				|  _ -> 
				  ...
				  let arg_nts_list = ndetval_list_of_il_arg_list 
				    arg_nts_list in
				  
				   (* List.map ( fun s-> interpret_c_scal_to_cnt sslv.ssl_part s ) *)
				  let nts_lvals = Compile_2_nts.make_ntsvars_of_ptrvar v.vname in
				  let cnt_trans_label = 
				    Flatac_ndet_nts_support_types.ND_CntGenCall(funname,Some(nts_lvals),arg_nts_list) in
				  ...
				  *)
				  ...
				  let mem_access_failure_cond = 
				    Nts_generic.neg_of_genrel mem_access_cond 
				  in
				  
				  let mem_access_failure_trans_label =
				    Flatac_ndet_nts_support.make_nd_guard_of_det_relation mem_access_failure_cond in
				  let failure_absdom = create_validity_abstdomain () in
				  set_heap_to_top failure_absdom.ssl_part;
			  ...
			  begin
			    match alias_tname with
				  ...
				  let arg_nts_list =
				   ...
				   (* List.map ( fun s-> interpret_c_scal_to_cnt sslv.ssl_part s ) *)
				  let nts_lvals = 
				    Flatac_ndet_nts_support.make_ntsvars_of_intvars v.vname 
				  in
				  let nts_lvals = Some(nts_lvals) in
				  let arg_list =  ndetval_list_of_il_arg_list
				    arg_nts_list 
				  in
				  let cnt_trans_label = 
				    Flatac_ndet_nts_support.make_nd_fun_call funname nts_lvals arg_list 
				  in
				 ...
				 (* let mem_access_trans_label =
				  ...
				  in
				  let mem_access_failure_cond =
				    Nts_generic.neg_of_genrel mem_access_cond in
				  let mem_access_failure_trans_label =
				    Flatac_ndet_nts_support.make_nd_guard_of_det_relation mem_access_failure_cond in
				  ...
				  let msg= 
				    Format.sprintf "[next_on_ssl_instr] Integer type Var : %s = %s : %s \n[next_on_ssl_instr] argument list --- \n "  (v.vname) (pprint_cil_exp exp1)( pprint_ciltypes v.vtype)  in (*(Nts.cnt_pprint_translabel cnt_trans_label) in*)
				Format.printf "%s" msg;
				 failtransit::((sslv,mem_access_trans_label::(cnt_trans_label::[]))::[])
				...
				begin
				  ...
				  match alias_tname with
					...
					compile_param_list_2_cnt_list 
				      ...
				      let nts_lvals =
					Flatac_ndet_nts_support.make_ntsvars_of_intvars v.vname in
				      let nts_lvals = Some(nts_lvals) in
				      let arg_list = ndetval_list_of_il_arg_list
					arg_nts_list  
				      in
				      let cnt_trans_label =
					Flatac_ndet_nts_support.make_nd_fun_call funname nts_lvals arg_list in
		...
		begin
		  ...
		  match f.vname with
			...
			(
			  match f.vtype with
			    ...
			    | _ -> let vname = 
				   ...
				   let nts_lvals = Flatac_ndet_nts_support.make_ntsvars_of_intvars vname in
				   Some(nts_lvals)
			);
		      in
		      let mem_access_cond = 
			(mem_guards_of_funcall_arg_list sslv lparam) in
		      let arg_list = ndetval_list_of_il_arg_list arg_nts_list
		      in
		      let cnt_trans_label = 
			Flatac_ndet_nts_support.make_nd_fun_call 
			  funname ret_nts_var_opt arg_list in
			...
			(sslv,cnt_trans_label::(mem_access_trans_label::[])) in
		      ...
		      let mem_access_failure_cond = Nts_generic.neg_of_genrel
			mem_access_cond in
		      let mem_access_failure_trans_label =
			Flatac_ndet_nts_support.make_nd_guard_of_det_relation
			  mem_access_failure_cond in
...
let next_on_ssl_nts (mid : global_mem_manager ) (sslv  ) (skind : Cil_types.stmtkind )   =
  match skind with 
	...
	match ret_value_type with 
		...
		let ptr_offset_var = 
		  Compile_2_nts.offset_var_of_varname "ret_val_" in
		let valid_ret_var = Compile_2_nts.valid_var_of_varname 
		  "ret_val_" in
		let cnt_affect_offset = 
		  Flatac_ndet_nts_support.make_affect_to_det_var_from_ndet_supp_cnt_val ptr_offset_var 
		    cnt_offset
		in
		...
		let valid_of_ret = 
		  (match valid_sym_expr with
		      DKvarValid ->
			let tlabel = 
			  Flatac_ndet_nts_support.havocize_var_list 
			    (valid_ret_var::[]) in
			tlabel::[]
		  (* CntHavoc(NtsIVar("validity__ret_val__")::[])*)
		    
		  | TruevarValid -> 
		    begin
		      let aff = 
			Nts_generic.affect_aexpr_to_nts_genrel_var 
			  valid_ret_var Nts_generic.one 
		      in
		      let aff_nd_guard = 
			Flatac_ndet_nts_support.make_nd_guard_of_det_relation 
			  aff 
		    ...
		    (*CntAffect(NtsIVar("validity__ret_val__"),CntCst(My_bigint.one))*)
		    end
		  | FalsevarValid -> 
		    begin
		      let aff = 
			Nts_generic.affect_aexpr_to_nts_genrel_var 
			  valid_ret_var Nts_generic.zero 
		      in
		      let aff_nd_guard = 
			Flatac_ndet_nts_support.make_nd_guard_of_det_relation 
			  aff 
		      in
		      aff_nd_guard::[]
		  (*CntAffect(NtsIVar("validity__ret_val__"),CntCst(My_bigint.zero)) *)
		    end
		  )
		in  
		let cnt_guard_aff_offset = 
		  Flatac_ndet_nts_support.make_nd_guard_of_relation
		    cnt_affect_offset 
		in
		(sslv , (valid_of_ret@(cnt_guard_aff_offset::[]))) :: []
	      end
	  | _->
	    begin
	      let cnt_exp =  compile_cil_exp_2_cnt sslv expr in
	      
	      let ret_var = Nts_generic.make_nts_genvar "ret_val_" NtsIntType in
	      let ret_aff = 
