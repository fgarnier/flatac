...
struct
  ...
  class cfgVisitor ( prj : Project.t ) 
	...
	self#_getUID sid abstraction _visitedNodes 0
	...
	if not (List.exists ( fun e -> e = nodeSemantic ) _visitedNodes ) then 
      ...
      method buildNodeList ( funInfo : Cil_types.fundec ) frontEnd =
	_currentECFG <- [];
